	...
	<c:if test="${not empty video}">
		<form action="video" method="post">
			<input type="hidden" name="act" value="update"> <input
				...
				ID:</label> <span>${video.id}</span><br> <br> <label for="title">제목:</label><br>
			<input type="text" id="title" name="title" value="${video.title}"
				required><br> <br> <label for="part">부위:</label><br>
			<input type="text" id="part" name="part" value="${video.part}"
				required><br> <br> <label for="channelName">채널명:</label><br>
			<input type="text" id="channelName" name="channelName"
				value="${video.channelName}" required><br> <br> <label
				for="url">URL:</label><br> <input type="text" id="url"
				name="url" value="${video.url}" required><br> <br>
		...
		<c:if test="${not empty sessionScope.userId}">
			<form action="${pageContext.request.contextPath}/review"
				...
				<input type="hidden" name="act" value="write"> <input
					type="hidden" name="videoId" value="${video.id}"> <label
					for="reviewContent">리뷰 작성:</label><br>
				<textarea id="reviewContent" name="content" rows="3" cols="50"
					required></textarea>
				<br>
		...
		<c:if test="${empty sessionScope.userId}">
			<p>
				리뷰를 작성하려면 <a
					href="${pageContext.request.contextPath}/user?act=loginform">로그인</a>하세요.
			</p>
		</c:if>
